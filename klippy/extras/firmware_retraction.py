...
class FirmwareRetraction:
    def __init__(self, config):
        self.config = config
        if (config.getboolean("use_zhop", True)):
            self.fw_retraction = FirmwareRetractionZHop(config)
        else:
            self.fw_retraction = FirmwareRetractionOld(config)

    def get_status(self, eventtime):
        return self.fw_retraction.get_status(eventtime)

class FirmwareRetractionOld:
    def __init__(self, config):
